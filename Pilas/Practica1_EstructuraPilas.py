...
class PilaVisual:
    ...
    def dibujar(self, extra=None):
        ...
        self.canvas.create_text(x_base + self.ancho//2, 30,
                                text=self.nombre, font=("Comic Sans MS", 12, "bold"), fill=borde_color)
        self.canvas.create_text(x_base + self.ancho//2, self.base_y + 50,
                                text=f"{len(self.items)}/{self.max_elementos}",
                                font=("Arial", 10, "bold"), fill=self.color_tema["texto"])
        self.canvas.create_line(x_base, self.base_y + 20, x_base + self.ancho, self.base_y + 20,
        ...
        for i, elemento in enumerate(self.items):
            ...
            self.rounded_rect(x1, y1, x2, y2, 15,
                              fill=self.color_tema["fill"], outline=self.color_tema["outline"], width=2)
        ...
        if self.items:
            ...
            self.canvas.create_text(x_base + self.ancho + 40, cima_y,
                                    text="⬅", fill="#ff006e", font=("Arial", 16, "bold"))
    ...
    def animar_apilar(self, elemento):
        ...
        for paso in range(pasos):
            ...
            x2, y2 = x1 + self.ancho, y1 + self.alto
            self.rounded_rect(x1, y1, x2, y2, 15, fill="#90e0ef", outline="#0077b6", width=2)
            self.canvas.create_text((x1+x2)//2, (y1+y2)//2,
                                    text=elemento, font=("Comic Sans MS", 10, "bold"), fill="white")
            self.canvas.update()
            time.sleep(0.03)
    ...
    def animar_desapilar(self):
        ...
        for paso in range(pasos):
            ...
            y1 = self.base_y - (len(self.items)+1)*self.alto - desplazamiento
            x2, y2 = x1 + self.ancho, y1 + self.alto
            self.rounded_rect(x1, y1, x2, y2, 15, fill="#ff595e", outline="#6a040f", width=2)
            self.canvas.create_text((x1+x2)//2, (y1+y2)//2,
                                    text=elemento, font=("Comic Sans MS", 10, "bold"), fill="white")
            self.canvas.update()
            time.sleep(0.04)
...
class GestorMultiPilas:
    ...
    def setup_ui(self):
        ...
        self.canvas = tk.Canvas(self.root, width=900, height=520, bg="#fdf6f9", highlightthickness=0)
        ...
        self.combo_pilas = tk.OptionMenu(frame_controles, self.var_pila, "", command=self.seleccionar_pila_combo)
        ...
        tk.Button(frame_ops, text="👁 Ver Cima", font=("Comic Sans MS", 10, "bold"),
                  bg="#a0c4ff", fg="white", command=self.ver_cima, width=12).grid(row=0, column=2, padx=3)
        tk.Button(frame_ops, text="🗑 Vaciar", font=("Comic Sans MS", 10, "bold"),
        ...
        tk.Button(frame_gestion, text="🗑 Eliminar Pila", font=("Comic Sans MS", 10, "bold"),
        ...
        tk.Button(frame_gestion, text="▶ Ejecutar tarea",
                  font=("Comic Sans MS", 10, "bold"),
                  bg="#80ed99", fg="white",
                  command=self.ejecutar_simulacion_tarea, width=15).grid(row=0, column=3, padx=3)

        self.info_label = tk.Label(self.root, text="", font=("Comic Sans MS", 10, "italic"),
                                   fg="#7209b7", bg="#fdf6f9")
    ...
    def crear_nueva_pila(self):
        nombre = simpledialog.askstring("Nueva Pila", "Nombre de la pila:")
        if nombre:
    ...
    def mover_elemento(self):
        # Stub sencillo: mueve tope de la pila activa a otra elegida
        if not self.pila_seleccionada:
            return
        origen = self.pilas[self.pila_seleccionada]
        if not origen.items:
            messagebox.showinfo("Pila vacía", "No hay elemento que mover")
            return
        destinos = [n for n in self.pilas.keys() if n != self.pila_seleccionada]
        if not destinos:
            messagebox.showinfo("Sin destino", "Crea otra pila para mover elementos")
            return
        destino_nombre = simpledialog.askstring("Mover a otra pila", f"Destino ({', '.join(destinos)}):")
        if destino_nombre not in self.pilas:
            return
        destino = self.pilas[destino_nombre]
        if len(destino.items) >= destino.max_elementos:
            messagebox.showwarning("Pila llena", "El destino está lleno")
            return
        x = origen.items.pop()
        destino.items.append(x)
        self.info_label.config(text=f"🔄 Movido '{x}' de '{self.pila_seleccionada}' a '{destino_nombre}'")
        self.redibujar_todo()
    ...
    def actualizar_combo(self):
        ...
        for nombre in self.pilas.keys():
            menu.add_command(label=nombre,
                             command=lambda n=nombre: self.var_pila.set(n) | self.seleccionar_pila_combo(n))
        if self.pila_seleccionada:
    ...
    def apilar(self):
        ...
        if len(pila.items) >= pila.max_elementos:
            messagebox.showwarning("Pila llena", f"La pila '{self.pila_seleccionada}' está llena")
            return
    ...
    def ver_cima(self):
        ...
        if not pila.items:
            ...
            return
        cima = pila.items[-1]
        self.info_label.config(text=f"👁 peek() de '{self.pila_seleccionada}': '{cima}'")
    ...
    def vaciar(self):
        ...
        while pila.items:
            ...
            time.sleep(0.1)
        self.redibujar_todo()
        self.info_label.config(text=f"🗑 Pila '{self.pila_seleccionada}' vaciada")

    # Simulación de la tarea: a–i
    def ejecutar_simulacion_tarea(self):
        if not self.pila_seleccionada:
            return
        pila = self.pilas[self.pila_seleccionada]

        # Secuencia del enunciado (LIFO):
        # a) push X, b) push Y, c) pop, d) pop, e) pop (underflow),
        # f) push V, g) push W, h) pop, i) push R
        ops = [
            ("push", "X"), ("push", "Y"),
        ...
        def paso(i=0):
            if i >= len(ops):
                self.info_label.config(text=f"✅ Fin: {len(pila.items)} elementos. Contenido: {pila.items}")
                self.redibujar_todo()
                return
            ...
            if accion == "push":
                if len(pila.items) >= pila.max_elementos:
                    messagebox.showwarning("Pila llena", "No se puede apilar (overflow)")
                else:
                    pila.animar_apilar(val)
                    self.info_label.config(text=f"▶ PUSH '{val}'")
            else:  # pop
                if not pila.items:
                    messagebox.showinfo("Pila vacía", "No se puede desapilar (underflow)")
                else:
                    x = pila.animar_desapilar()
                    self.info_label.config(text=f"▶ POP '{x}'")

            self.redibujar_todo("STEP")
            # Programa siguiente paso sin congelar UI
            self.root.after(350, lambda: paso(i+1))
